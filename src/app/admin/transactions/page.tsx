...
export default function AdminTransactions() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((t) => {
      ...
      const matchesProperty = propertyFilter === 'All' || t.property.toLowerCase().includes(String(propertyFilter).toLowerCase());
      const matchesUser = userFilter === 'All' || t.user === userFilter;
      const matchesDate = startDate ? new Date(t.date) >= startDate : true;
      return matchesSearch && matchesType && matchesStatus && matchesProperty && matchesUser && matchesDate;
    });
  }, [transactions, searchTerm, typeFilter, statusFilter, propertyFilter, userFilter, dateRange]);

  const uniqueUsers = useMemo(() => Array.from(new Set(transactions.map(t => t.user))), [transactions]);
  const uniqueProperties = useMemo(() => Array.from(new Set(transactions.map(t => t.property))), [transactions]);
  ...
  const handleExport = () => {
    const csvContent = 'data:text/csv;charset=utf-8,' +
      ...
      filteredTransactions.map(t => (
        `${t.id},${new Date(t.date).toLocaleString()},${t.user},${t.property},${t.type},${t.amount},${t.status},${t.shares}`
      )).join('\n');
        ...
        <main className="p-4 sm:p-6 lg:p-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
              ...
              <div>
                <select
                  value={dateRange}
                  onChange={(e) => setDateRange(e.target.value as 'All' | 'Today' | 'Week' | 'Month' | 'Quarter')}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy focus:border-transparent"
                >
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div>
                <select
                  ...
                  onChange={(e) => setPropertyFilter(e.target.value as 'All' | string)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy focus:border-transparent"
                >
              ...
              <div>
                <select
                  value={userFilter}
                  onChange={(e) => setUserFilter(e.target.value as 'All' | string)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy focus:border-transparent"
                >
              ...
              <div>
                <button className="flex items-center px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors w-full justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            {[
              ...
              { title: 'Total Volume', value: '$' + filteredTransactions.reduce((sum, t) => sum + (t.amount || 0), 0).toLocaleString(), icon: DollarSign, color: 'bg-green-500' },
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredTransactions.map((transaction, index) => (
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-navy">{transaction.id}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm"><span className={`font-medium ${getTypeColor(transaction.type)}`}>{transaction.type}</span></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-navy">${(transaction.amount || 0).toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{transaction.shares}</td>
                      <td className="px-6 py-4 whitespace-nowrap"><span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(transaction.status)}`}>{transaction.status}</span></td>
                    </motion.tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mt-6 flex items-center justify-between">
            <div className="text-sm text-gray-700">Showing {filteredTransactions.length} of {transactions.length} transactions</div>
            <div className="flex items-center space-x-2">
              <button className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Previous</button>
              <button className="px-3 py-2 bg-navy text-white rounded-lg hover:bg-gray-800 transition-colors">1</button>
